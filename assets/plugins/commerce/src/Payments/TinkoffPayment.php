...
class TinkoffPayment extends Payment implements \Commerce\Interfaces\Payment
{
	...
	public function getPaymentLink() {
		...
		if (isset($this->_error)) {
			if ($this->debug) {
				$this->modx->logEvent(0, 3, $this->_error, 'Commerce Tinkoff Payment Error');
			}
	...
	protected function request($method, $data) {
		...
		if ($curl = curl_init()) {
			...
			if ($json) {
				...
				if ($this->debug) {
					$this->modx->logEvent(0, 1, $result, 'Commerce Tinkoff Response');
				}
				
				return $result;
			} 
			
			$this->_error = '<pre>'.print_r($curl_info, true);
			return false;
